...
public class ProfileBuilder {
  ...
  private static CompoundProfileSegment CreateLinear(final Vector start, final Vector dir, double startSpeed, double endSpeed, DrivetrainConfig config) {

    CompoundProfileSegment out = new CompoundProfileSegment();
    final PiecewiseFunction<Derivatives<Double>> magProfile = CreateVelocityChange(startSpeed, endSpeed, config);

    final Vector normalDir = dir.normalize();

    for (BoundedFunction<Derivatives<Double>> function : magProfile.functions) {

      final SingleBoundedFunction<Derivatives<Vector>> vectorFunction =
          new SingleBoundedFunction<>(t -> new Derivatives<>(
          start.add(normalDir.scale(function.get(t).position)),
          start.add(normalDir.scale(function.get(t).velocity)),
          start.add(normalDir.scale(function.get(t).acceleration)),
          start.add(normalDir.scale(function.get(t).jerk))), function.lowerBound(), function.upperBound());

      out.add(new Linear(vectorFunction));
    }
  ...
  private static Linear CreateLinear(SingleBoundedFunction<Derivatives<Double>> profile, Vector start, Vector dir) {
    ...
    return new Linear(function);
  }

  private static CompoundProfileSegment CreateDisplacement(Vector start, Vector end, double startSpeed, double endSpeed, double maxSpeed, DrivetrainConfig config) {
    final double dist = end.dist(start);
